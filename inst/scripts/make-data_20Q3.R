...
##########################################
## depmap `metadata_20Q3` dataset
##########################################

### loading data (downloading .csv file from online source)
url_39 <- "https://ndownloader.figshare.com/files/24613394"
sample_info <- read_csv(url_39)

### DepMap-2020Q3-celllines.csv data renamed to `metadata`
metadata_20Q3 <- sample_info
# names(metadata_20Q3)

#### Rename `metadata` columns to contain underscores and be in snake case
## note: "metadata_20Q3" has different columns than "metadata_20Q3"
names(metadata_20Q3)[1:25] <-c(
    "depmap_id", "stripped_cell_line_name", "cell_line", "aliases", "cosmic_id",
    ...
    "sanger_id", "WTSI_master_cell_ID", "additional_info", "lineage",
    "lineage_subtype", "lineage_sub_subtype", "lineage_molecular_subtype")

### saving cleaned and converted `metadata` data as .rda file
save(metadata_20Q3, file = "../eh_data/metadata_20Q3.rda",
     compress = "xz", compression_level = 9)

##########################################
## `depmap_id_to_name_20Q3 map `depmap_id` to `cell_line`
##########################################

##  generation of `metadata` subset `depmap_id_to_name_20Q3`
...
depmap_id_to_name_20Q3 <- metadata_20Q3 %>% dplyr::select(depmap_id, cell_line)

##########################################
## depmap `mutationCalls_20Q3` dataset
##########################################

### loading data (downloading .csv file from online source)
url_40 <- "https://ndownloader.figshare.com/files/24613355"
CCLE_mutations_20Q3 <- read_tsv(url_40)

### depmap_20Q3_mutation_calls data renamed to `mutationCalls`
mutationCalls_20Q3 <- CCLE_mutations_20Q3

## note: "mutationCalls_20Q3" has different columns than "mutationCalls_19Q1"
## the variable "VA_WES_AC" is no longer present in this dataset, unlike
## previous releases (e.g. 19Q1)! 
names(mutationCalls_20Q3)[1:34] <- c(
    ...
    "tcga_hsCnt", "is_cosmic_hotspot", "cosmic_hsCnt", "ExAC_AF", "CGA_WES_AC",
    "sanger_WES_AC", "sanger_recalib_WES_AC", "RNAseq_AC", "HC_AC", "RD_AC",
    "WGS_AC", "var_annotation","depmap_id")

### rearrange columns into same column format as other datasets
mutationCalls_20Q3 <- mutationCalls_20Q3 %>%
    dplyr::select(depmap_id, everything())

### saving cleaned and converted `mutationCalls` data as .rda file
save(mutationCalls_20Q3, file = "../eh_data/mutationCalls_20Q3.rda",
     compress = "xz", compression_level = 9)

##########################################
## depmap `copyNumber_20Q3` dataset
##########################################

### loading data (downloading .csv file from online source)
url_41 <- "https://ndownloader.figshare.com/files/24613352"
CCLE_gene_cn_20Q3 <- read_csv(url_41)

### public_20Q3_gene_cn.csv data renamed to `copyNumber`
copyNumber_20Q3 <- CCLE_gene_cn_20Q3

### rename column first column to "depmap_id"
names(copyNumber_20Q3)[1] <- "depmap_id"

### gather into long form on columns: `depmap_id`, `gene`, `logCopyNumber`
copyNumber_20Q3_long <- gather(copyNumber_20Q3, gene, log_copy_number,
                               -depmap_id)

### mutate gene column into `gene_name` and `entrez_id`
copyNumber_20Q3_long <- copyNumber_20Q3_long %>% 
    mutate(entrez_id = gsub("&", ";", sub("\\)", "", sub("^.+ \\(", "", gene))),
           gene_name = gsub("&", ";", sub(" \\(.+\\)$", "", gene))) 

### left_join `copyNumber` & `depmap_id_to_name_20Q3` on `depmap_id`, 
## `cell_line`     
copyNumber_20Q3 <- copyNumber_20Q3_long %>% 
    left_join(depmap_id_to_name_20Q3, by = c("depmap_id" = "depmap_id"))

### rearrange columns into same column format as other datasets
copyNumber_20Q3 <- copyNumber_20Q3 %>% 
...
save(copyNumber_20Q3, file = "../eh_data/copyNumber_20Q3.rda",
     compress = "xz", compression_level = 9)

##########################################
## depmap `crispr_20Q3` dataset
##########################################

### loading data (downloading .csv file from online source)
url_42 <- "https://ndownloader.figshare.com/files/24613292"
Achilles_gene_effect_20Q3 <- read_csv(url_42)

### gene_effect_corrected.csv data renamed to `crispr`
crispr_20Q3 <- Achilles_gene_effect_20Q3

### rename column first column to "depmap_id"
names(crispr_20Q3)[1] <-"depmap_id"

### gather cripsr into long form with columns: `depmap_id`, `gene`, `dependency`
crispr_20Q3_long <- gather(crispr_20Q3, gene, dependency, -depmap_id)

### mutate gene into `gene_name` and `entrez_id`
crispr_20Q3_long <- crispr_20Q3_long %>% 
    mutate(entrez_id = gsub("&", ";", sub("\\)", "", sub("^.+ \\(", "", gene))),
           gene_name = gsub("&", ";", sub(" \\(.+\\)$", "", gene))) 

### left_join `crispr_long` and `depmap_id_to_name` to add `cell_line` column   
crispr_20Q3 <- crispr_20Q3_long %>% left_join(depmap_id_to_name_20Q3, 
                                    by = c("depmap_id" = "depmap_id"))

### rearrange columns into same column format as other datasets
crispr_20Q3 <- crispr_20Q3 %>% dplyr::select(depmap_id, gene,
                                             dependency, entrez_id,
                                             gene_name, cell_line) %>%
                                             type_convert(cols(entrez_id = "i"))

### saving cleaned and converted `crispr` data as .rda file
save(crispr_20Q3, file = "../eh_data/crispr_20Q3.rda",
     compress = "xz", compression_level = 9)

##########################################
## depmap `TPM_20Q3` dataset
##########################################

### loading data (downloading .csv file from online source)
url_43 <- "https://ndownloader.figshare.com/files/24613325"
CCLE_expression_full_20Q3 <- read_tsv(url_43)

### CCLE_depMap_20Q3_TPM.csv data renamed to `TPM`
TPM_20Q3 <- CCLE_expression_full_20Q3
# names(TPM_20Q3)

### rename column first column to "depmap_id"
names(TPM_20Q3)[1] <-"depmap_id"

### gather `TPM` into long form on columns: `depmap_id`, `gene`, `expression`
TPM_20Q3_long <- gather(TPM_20Q3, gene, rna_expression, -depmap_id)

### mutate gene into gene_name and ensembl_id
TPM_20Q3_long <- TPM_20Q3_long %>% 
    mutate(ensembl_id = gsub("&", ";", sub("\\)", "", sub("^.+ \\(", "",gene))),
           gene_name = gsub("&", ";", sub(" \\(.+\\)$", "", gene))) 

### left_join join `TPM` and `depmap_id_to_name_20Q3` to add `cell_line` column
TPM_20Q3 <- TPM_20Q3_long %>% left_join(depmap_id_to_name_20Q3, 
                              by = c("depmap_id" = "depmap_id"))
