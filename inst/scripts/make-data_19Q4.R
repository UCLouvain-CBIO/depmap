...
##########################################
## depmap `RPPA_19Q4` dataset
##########################################

## data cleaning of `RPPA` dataset

### loading data (downloading .csv file from online source)
# url_28 <- "https://depmap.org/portal/download/api/download/external?file_name=ccle%2Fccle_2019%2FCCLE_RPPA_20181003.csv"
# CCLE_RPPA_20180123_19Q4 <- read_csv(url_28)
# 
# ### CCLE_RPPA_20180123.csv data renamed to `RPPA`
# RPPA_19Q4 <- CCLE_RPPA_20180123_19Q4
# # head(RPPA_19Q4)
# 
# ### rename column first column to "cell_line"
# names(RPPA_19Q4)[1] <-"cell_line"
# 
# ### gather cell line columns and split gene name into gene_name and entrez_id
# RPPA_19Q4_long <- RPPA_19Q4 %>% gather(antibody, expression, -cell_line)
# 
# ### join cell_line name    
# RPPA_19Q4 <- RPPA_19Q4_long %>% left_join(depmap_id_to_name_19Q4, 
#                                 by = c("cell_line" = "cell_line"))
# 
# ### rearrange columns into same column format as other datasets
# RPPA_19Q4 <- RPPA_19Q4 %>% dplyr::select(cell_line, antibody, expression,
#                                             depmap_id) 
# 
# ### visual check
# # head(RPPA_19Q4)
...
# save(RPPA_19Q4, file="../eh_data/RPPA_19Q4.rda", compress="xz",
#      compression_level=9)
...
##########################################
## depmap `rnai_19Q4` dataset
##########################################

## data cleaning of `rnai` dataset

# ### loading data (downloading .csv file from online source)
# url_29 <- "https://ndownloader.figshare.com/files/13515395"
# D2_combined_gene_dep_scores_19Q4 <- read_csv(url_29)
# 
# ### D2_combined_genetic_dependency_scores.csv data renamed to `rnai`
# rnai_19Q4 <- D2_combined_gene_dep_scores_19Q4
# # head(rnai_19Q4)
# 
# ### rename column first column to "gene"
# names(rnai_19Q4)[1] <- "gene"
# 
# ### gather rnai into long form with columns: depmap_id, gene and dependency
# rnai_19Q4 <- gather(rnai_19Q4, key = cell_line, value = dependency, -gene) %>%
#     mutate(entrez_id = gsub("&", ";", sub("\\)", "", sub("^.+ \\(", "", gene))),
#            gene_name = gsub("&", ";", sub(" \\(.+\\)$", "", gene))) %>%
#     left_join(depmap_id_to_name_19Q4, by = c("cell_line" = "cell_line"))
# 
# ### rearrange columns into same column format as other datasets
# rnai_19Q4 <- rnai_19Q4 %>% dplyr::select(gene, cell_line, dependency, entrez_id,
#                                          gene_name, depmap_id) %>%
#                                          type_convert(cols(entrez_id = "i"))
# ### visual check
# # head(rnai_19Q4)
...
# save(rnai_19Q4, file="../eh_data/rnai_19Q4.rda", compress="xz",
#      compression_level=9)
...
##########################################
## depmap `drug_sensativity_19Q4` dataset
##########################################

## data cleaning of `drug_sensativity` dataset

### loading data (downloading .csv file from online source)
# url_30 <- "https://depmap.org/portal/download/api/download/external?file_name=drug%2Fprimary-screen-e5c7.7%2Fprimary_replicate_collapsed_logfold_change.csv"
# primary_replicate_collapsed_logfold_change_19Q4 <- read_csv(url_30)
#
# ### primary_replicate_collapsed_logfold_change_19Q4.csv data renamed to
# ### `drug_sensativity_19Q4`
# drug_sensativity_19Q4 <- primary_replicate_collapsed_logfold_change_19Q4
# # head(drug_sensativity_19Q4)
#
# ### rename column first column to "depmap_id"
# names(drug_sensativity_19Q4)[1] <- "depmap_id"
#
# ### gather drug_sensativity into long form with columns: depmap_id, compound and
# ### dependency
# drug_sensativity_19Q4 <- gather(drug_sensativity_19Q4, key = compound,
#                                 value = dependency, -depmap_id) %>%
#                                 left_join(depmap_id_to_name_19Q4,
#                                           by = c("depmap_id" = "depmap_id"))
#
# ### rearrange columns into same column format as other datasets
# drug_sensativity_19Q4 <- drug_sensativity_19Q4 %>%
...
# save(drug_sensativity_19Q4, file="../eh_data/drug_sensativity_19Q4.rda",
#      compress="xz", compression_level=9)
