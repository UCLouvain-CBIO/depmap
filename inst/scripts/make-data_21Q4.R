...
metadata_21Q4 <- read_csv(file = "https://ndownloader.figshare.com/files/31316011")

#### Rename `metadata` columns to contain underscores and be in snake case
## note: "metadata_21Q4" has different columns than "metadata_21Q4"
...
save(metadata_21Q4, file = "../eh_data/metadata_21Q4.rda",
      compress = "xz", compression_level = 9)

##########################################
## `dep_2_name_21Q4 map `depmap_id` to `cell_line`
##########################################

##  generation of `metadata` subset `dep_2_name_21Q4`
...
### loading data (downloading .csv file from online source)
mutationCalls_21Q4 <- read_csv(file = "https://ndownloader.figshare.com/files/31315930")

## note: "mutationCalls_21Q4" has different columns than "mutationCalls_19Q1"
...
save(mutationCalls_21Q4, file = "../eh_data/mutationCalls_21Q4.rda",
     compress = "xz", compression_level = 9)
...
### loading data (downloading .csv file from online source)
copyNumber_21Q4 <- read_csv("https://ndownloader.figshare.com/files/31315924")

### rename column first column to "depmap_id"
names(copyNumber_21Q4)[1] <- "depmap_id"

### gather into long form on columns: `depmap_id`, `gene`, `logCopyNumber`
copyNumber_21Q4 %>%
  gather(gene, log_copy_number, -depmap_id) -> copyNumber_21Q4_long

### mutate gene column into `gene_name` and `entrez_id`
copyNumber_21Q4_long %>%
    mutate(entrez_id = gsub("&", ";", sub("\\)", "", sub("^.+ \\(", "", gene))),
           gene_name = gsub("&", ";", sub(" \\(.+\\)$", "", gene))
           ) -> copyNumber_21Q4_long

### left_join `copyNumber` & `dep_2_name_21Q4` on `depmap_id`,
 copyNumber_21Q4_long %>%
    left_join(dep_2_name_21Q4,
              by = c("depmap_id" = "depmap_id")) -> copyNumber_21Q4

### rearrange columns into same column format as other datasets
copyNumber_21Q4 %>% 
  select(depmap_id, gene, log_copy_number, entrez_id, gene_name, cell_line) %>%
  type_convert(cols(entrez_id = "i")) -> copyNumber_21Q4

### saving cleaned and converted `copyNumber` data as .rda file
save(copyNumber_21Q4, file = "../eh_data/copyNumber_21Q4.rda",
     compress = "xz", compression_level = 9)
...
### loading data (downloading .csv file from online source)
crispr_21Q4 <- read_csv(file = "https://ndownloader.figshare.com/files/31315996")

### rename column first column to "depmap_id"
names(crispr_21Q4)[1] <-"depmap_id"

### gather cripsr into long form with columns: `depmap_id`, `gene`, `dependency`
crispr_21Q4 %>%
  gather(gene, dependency, -depmap_id) -> crispr_21Q4_long
...
crispr_21Q4_long %>%
  left_join(dep_2_name_21Q4, by = c("depmap_id" = "depmap_id")) -> crispr_21Q4

### rearrange columns into same column format as other datasets
crispr_21Q4 %>%
  select(depmap_id, gene, dependency, entrez_id, gene_name, cell_line) %>%
  type_convert(cols(entrez_id = "i")) -> crispr_21Q4

### saving cleaned and converted `crispr` data as .rda file
save(crispr_21Q4, file = "../eh_data/crispr_21Q4.rda",
     compress = "xz", compression_level = 9)
...
### loading data (downloading .csv file from online source)
TPM_21Q4 <- read_csv(file = "https://ndownloader.figshare.com/files/31315882")

### rename column first column to "depmap_id"
names(TPM_21Q4)[1] <-"depmap_id"

### gather `TPM` into long form on columns: `depmap_id`, `gene`, `expression`
TPM_21Q4 %>%
  gather(gene, rna_expression, -depmap_id) -> TPM_21Q4_long
...
### left_join join `TPM` and `dep_2_name_21Q4` to add `cell_line` column
TPM_21Q4_long %>%
  left_join(dep_2_name_21Q4, by = c("depmap_id" = "depmap_id")) -> TPM_21Q4

### rearrange columns into same column format as other datasets
TPM_21Q4 %>%
    select(depmap_id, gene, rna_expression, entrez_id, gene_name, cell_line) %>%
    type_convert(cols(entrez_id = "i")) -> TPM_21Q4

### saving cleaned and converted `TPM` data as .rda file
save(TPM_21Q4, file = "../eh_data/TPM_21Q4.rda", compress = "xz",
     compression_level = 9)
