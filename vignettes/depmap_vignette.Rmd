...
ggplot(depAllGenes) + aes(ID, Dependency_Score, color = df3$Dependency) + 
    geom_point() +  scale_colour_viridis() + 
    ...
    ggtitle("Plot of the greatest dependency scores for each cell line") +
...
# add cellLines column 
#cellToID2 <-cbind("cellLine" = paste(cellLinesD2, seq_len(nrow(cellToID))), cellToID)
#View(cellToID2[1:10, 1:10])
...
# compare columns of geneIDcrispr and noNAdepIDrnai 

#a <-names(noNAdepIDrnai)
#[1]  1677 17311
#b <-names(geneIDcrispr)
#[1]  1677 16637

# this is all not working below 

#compareCols <- a[!(a %in% b)]
#compareCols <-as.character(compareCols)
#class(compareCols)
#View(compareCols)

#newData <- subset(geneIDcrispr, select = compareCols) 
#dim(newData)

#crisprAndRnai <- geneIDcrispr[, -(names(geneIDcrispr) %in% compareCols)]
#dim(crisprAndRnai)

# remove genes tested in CRISPR dataset that are not in RNAI dataset
#crisprAndRnai <- subset(geneIDcrispr, select = -c(compareCols))
...
rnaiNoNaPIK3C3 <- subset(noNAdepIDrnai, !is.na(`PIK3C3 (5289)`))
#dim(noNArnaiPIK3C3)
# [1]   709 17311 
#View(noNArnaiPIK3C3[1:10, 1:10])


geneIDcrispr$`PIK3C3 (5289)`

crisprNoNaPIK3C3 <- subset(geneIDcrispr, !is.na(`PIK3C3 (5289)`))
#dim(crisprNoNaPIK3C3)
# [1]   558 17637
#View(crisprNoNaPIK3C3[1:10, 1:10])

# how do we merge the CRISPR and RNAi columns? 
# first sort by depmapID
rnaiNoNaPIK3C3 <- rnaiNoNaPIK3C3[order(depmapID),] 
View(noNArnaiPIK3C3[1:10, 1:10])
crisprNoNaPIK3C3 <- crisprNoNaPIK3C3[order(depmapID),] 
View(crisprNoNaPIK3C3[1:10, 1:10])
View(crisprNoNaPIK3C3$`PIK3C3 (5289)`)

noNArnaiPIK3C3 <-cbind(noNAdepIDrnai$depmapID, noNAdepIDrnai$`PIK3C3 (5289)`)

# why are there so many NAs??????? 

# combine columns into one dataframe
PIK3C3 <-cbind(geneIDcrispr$`PIK3C3 (5289)`, noNAdepIDrnai$`PIK3C3 (5289)`)

#add col names to PIK3C3 to make the variables more clear
colnames(PIK3C3, do.NULL = FALSE)
colnames(PIK3C3) <- c("depCRISPR","depRNAi")
View(PIK3C3)
