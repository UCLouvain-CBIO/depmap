...
that cell line. The remaining columns consist of the numerical dependency score 
...
Much of the data that can be found within the 
...
We will demonstrate how to obtain the dependency scores corresponding to a
specific gene or cell lineage. For example, we would like to know what 
dependency is present for a well known human tumor suppressor gene, like `BRCA1`
for a breast cancer lineage such as `184A1_BREAST`, found in the `rnai` dataset. 
...
If we would like to obtain the highest and lowest dependency scores for `BRCA1`
along with the cell lines associated with those values:
...
```{r, eval=TRUE, echo=T}
# need to remove the columns with characters before finding max/min
...
Below shows a table which returns the 10 lowest values for dependency scores in 
the `rnai` data, along with corresponding gene and cell line for that value. 
...
rnaiMinCols <- (apply(rnai0Chars,MARGIN=2,which.min))

# Return min row index
rnaiMinRows <- (apply(rnai0Chars,MARGIN=1,which.min))

# lowest dependency scores for each column of `rnai`
lowDepScoresColRNAI <- apply(rnai0Chars, 1, function(x) {min(x, na.rm = TRUE)})
View(lowDepScoresColRNAI)

# lowest dependency scores for each column of `rnai`
lowDepScoresRowRNAI <- apply(rnai0Chars, 2, function(x) {min(x, na.rm = TRUE)})
View(lowDepScoresRowRNAI)

View(rnai[c(1:1, 1:10)])

#############################################3

# making a table that has Cell line, gene, dependency score

View(rnaiMinCols)
View(rnaiMinRows)
View(lowDepScoresRNAI)

```

Below are a series of plots to help illustrate structures in the data. 
