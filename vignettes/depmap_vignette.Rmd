...
packages <- c("knitr", "tidyr", "dplyr", "ggplot2", "plotly", "viridis", "tidyverse")
...
If you wish to see how the data was transformed, please see the dataCleaning.Rmd
vignette. 
...
names((CCLE_depMap_19Q1_TPM[c(1:10)]))
# [1] "X1"                         "TSPAN6 (ENSG00000000003)"   "TNMD (ENSG00000000005)"     "DPM1 (ENSG00000000419)"    
# [5] "SCYL3 (ENSG00000000457)"    "C1orf112 (ENSG00000000460)" "FGR (ENSG00000000938)"      "CFH (ENSG00000000971)"     
# [9] "FUCA2 (ENSG00000001036)"    "GCLC (ENSG00000001084)" 
# View(CCLE_depMap_19Q1_TPM[c(1:10)])
dim(CCLE_depMap_19Q1_TPM)
# [1]  1165 57821
```
...
names((CCLE_RPPA_20180123[c(1:10)]))
# [1] "X1"                     "14-3-3_beta"            "14-3-3_epsilon_Caution" "14-3-3_zeta"           
# [5] "4E-BP1"                 "4E-BP1_pS65"            "4E-BP1_pT37_T46"        "4E-BP1_pT70"           
# [9] "53BP1"                  "A-Raf_pS299_Caution"   
#View(CCLE_RPPA_20180123[c(1:10)])
dim(CCLE_RPPA_20180123)
# [1] 899 215
```
...
# [1] "X1"                                     "127399_SOFT_TISSUE"                    
# [3] "1321N1_CENTRAL_NERVOUS_SYSTEM"          "143B_BONE"                             
# [5] "184A1_BREAST"                           "184B5_BREAST"                          
# [7] "22RV1_PROSTATE"                         "2313287_STOMACH"                       
# [9] "600MPE_BREAST"                          "697_HAEMATOPOIETIC_AND_LYMPHOID_TISSUE"
...
# [1] "DepMap_ID"       "CCLE_Name"       "Aliases"         "COSMIC_ID"       
# [5] "Sanger.ID"       "Primary.Disease"  "Subtype.Disease" "Gender"   "Source"   
#View(depmap_19Q1_cell_lines)
dim(depmap_19Q1_cell_lines)
# [1] 1677    9
```
...
#[1] "X1"                     "Hugo_Symbol"            "Entrez_Gene_Id"         "NCBI_Build"            
#[5] "Chromosome"             "Start_position"         "End_position"           "Strand"                
#[9] "Variant_Classification" "Variant_Type"           "Reference_Allele"       "Tumor_Seq_Allele1"     
#[13] "dbSNP_RS"               "dbSNP_Val_Status"       "Genome_Change"          "Annotation_Transcript" 
#[17] "Tumor_Sample_Barcode"   "cDNA_Change"            "Codon_Change"           "Protein_Change"        
#[21] "isDeleterious"          "isTCGAhotspot"          "TCGAhsCnt"              "isCOSMIChotspot"       
#[25] "COSMIChsCnt"            "ExAC_AF"                "VA_WES_AC"              "CGA_WES_AC"            
#[29] "SangerWES_AC"           "SangerRecalibWES_AC"    "RNAseq_AC"              "HC_AC"                 
#[33] "RD_AC"                  "WGS_AC"                 "Variant_annotation"     "DepMap_ID"             
#View(depmap_19Q1_mutation_calls)
dim(depmap_19Q1_mutation_calls)
# [1] 1243145      36
```
...
# [1] "X1"               "A1BG (1)"         "A1CF (29974)"     "A2M (2)"          "A2ML1 (144568)"   "A3GALT2 (127550)"
# [7] "A4GALT (53947)"   "A4GNT (51146)"    "AAAS (8086)"      "AACS (65985)"  
...
# [1] "X1"                     "A1BG (1)"               "NAT2 (10)"              "ADA (100)"             
# [5] "CDH2 (1000)"            "AKT3 (10000)"           "GAGE12F (100008586)"    "ZBTB11-AS1 (100009676)"
# [9] "MED6 (10001)"           "NR2E3 (10002)"        
#View(public_19Q1_gene_cn[c(1:10)])
dim(public_19Q1_gene_cn)
#[1]  1604 23300
```
...
We have tried to preserve the original names of the [depmap](https://github.com/tfkillian/depmap/) 
.csv files, however many of these names are long, unwieldy and don't immediately 
give the user the accurate impression of what is contained in the data frame. We 
will give these datasets new names to address this. 
...
Older datasets can be called by `depmap_19Q1_mutation_calls` or by downloading 
the .csv directly from the [Broad Institute](https://depmap.org/portal/download/) 
portal.
...
cell lines for the RNAI dataset.
...
lowDepScoresCRISPR <- apply(gene_effect_corrected[c(2:17635)], 1, 
                        function(x) {min(x, na.rm = TRUE)})

lowDepGeneCRISPR <- apply(gene_effect_corrected[c(2:17635)], 1, 
                function(x) {depmin <-which.min(x)
                D2_combined_genetic_dependency_scores$X1[depmin]})
...
# turn row names into a column
#r3 <-rownames(lowDepGeneCRISPR)
#lowDepScoresCRISPR$Gene <- r3

#View(lowDepScoresCRISPR$Gene)

#View(r1)
#r2 <-rownames(lowDepGeneCRISPR)
#low_dep_gene$Cell_Line <- r2

# tidy up df, merge on Cell_Line
#df3 = merge(low_dep_gene, low_dep_scores, by.x=c("Cell_Line"), by.y=c("Cell_Line"))
...
Below is a plot of the lowest dependency scores for each gene and CL for the 
`gene_effect_corrected` dataset. 
 
```{r, eval=FALSE, echo=T}
# apply function to get mean dependency from all columns
gEc_avg <- apply(gene_effect_corrected[c(2:17635)], 1, function(x) {mean(x, na.rm = TRUE)})
...
#str(compDepCOPB1)
#set all NAs to 0 for compareCOPB1 so the dataframe can be visualized

# compareCOPB1$rnaiCOPB1 has to be turned into character because it throws 
# an annoying warning "invalid factor level, NA generated"
#compareCOPB1$rnaiCOPB1<- as.character(compareCOPB1$rnaiCOPB1)
#compareCOPB1[, 1:3][is.na(compareCOPB1[, 1:3])] <- 0

# compareCOPB1$rnaiCOPB1 converted back to numeric. Note: this is an inelegant
# solution, need to find something that works better 
#compareCOPB1$rnaiCOPB1<- as.numeric(compareCOPB1$rnaiCOPB1)

#convert compareCOPB1 to depCOPB1, and convert 0 values to NA
#depCOPB1 <- compareCOPB1
#depCOPB1[depCOPB1==0] <- NA
#depCOPB1

#Delete the rows associated with NA.
#compDepCOPB1<-depCOPB1[complete.cases(depCOPB1),]
#View(compDepCOPB1)
#dim(compDepCOPB1)
#[1] 402   3
# this means that there are only 402 cell lines where there are dependency 
...
# sort byDepIdCOPB1 by depmapID, lowest to highest
#byDepIdCOPB1 <- compDepCOPB1[order(compDepCOPB1$depmapID, decreasing=F),]
...
#varMutType <- depmap_19Q1_mutation_calls$Variant_Classification

# Don't map a variable to y
p3 <- ggplot(depmap_19Q1_mutation_calls, aes(x=factor(Variant_Classification)))+
  geom_bar(stat="Count", width=0.7, fill="steelblue")+theme_minimal() +
    ggtitle("Types of cancer mutation classifications in DepMap dataset") 
# Horizontal bar plot
p3 + coord_flip()
...
p4 <- ggplot(depmap_19Q1_mutation_calls, aes(x=factor(Variant_Type)))+
  geom_bar(stat="Count", width=0.7, fill="steelblue")+theme_minimal() +
    ggtitle("Types of cancer mutations present in DepMap dataset") 
# Horizontal bar plot
p4 + coord_flip()
...
Copy Number Note: Find out what the values of copy number are in public_19Q1_gene_cn
