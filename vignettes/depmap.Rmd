...
# display results of the depmap query
x
...
`ExperimentHub` metadata for the `depmap` datasets can be found by calling the
*.onLoad* function and setting to *metadata = TRUE* (note: this is not the same
metadata as the `metadata` found in the 19Q1 release)

```{r, show_onLoad_metadata}
depmap(metadata = FALSE) ## data are loaded
depmap(metadata = TRUE)  ## metadata are displayed
```


### Downloading [Broad Institute](https://depmap.org/portal/download/) data

If desired, the original wide form data from which the [depmap](https://github.com/UCLouvain-CBIO/depmap)
...
transformed and loaded into the [depmap](https://github.com/UCLouvain-CBIO/depmap)
