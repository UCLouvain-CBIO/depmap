...
# vignette
packages <- c("RCurl", "dplyr", "devtools")
...
depmap_19Q1_cell_lines  <- read.csv("https://depmap.org/portal/download/api/download/external?file_name=processed_portal_downloads%2Fdepmap-public-cell-line-metadata-183e.4%2FDepMap-2019q1-celllines_v2.csv")
...
If the *.csv* files are already downloaded, they may be loaded manually.

```{r, eval=FALSE, echo=T}
#load necessary libraries

#getwd()
setwd("~/R_projects/depmapr_csv_files")

# manually load 'D2_combined_genetic_dependency_scores' 
D2_combined_genetic_dependency_scores <- read.csv("~/R_projects/depmapr_csv_files/D2_combined_gene_dep_scores.csv")

# manually load  'gene_effect_corrected'
gene_effect_corrected <- read.csv("~/R_projects/depmapr_csv_files/gene_effect_corrected.csv")

# manually load 'depmap_19Q1_mutation_calls'
depmap_19Q1_mutation_calls <- read.csv("~/R_projects/depmapr_csv_files/depmap_19Q1_mutation_calls.csv")

# manually load 'depmap_19Q1_cell_lines'
depmap_19Q1_cell_lines <- read.csv("~/R_projects/depmapr_csv_files/DepMap-2019q1-celllines.csv")

# manually load 'public_19Q1_gene_cn'
public_19Q1_gene_cn <- read.csv("~/R_projects/depmapr_csv_files/public_19Q1_gene_cn.csv")

# manually load 'CCLE_RPPA_20180123'
CCLE_RPPA_20180123 <- read.csv("~/R_projects/depmapr_csv_files/CCLE_RPPA_20180123.csv")

# manually load 'CCLE_depMap_19Q1_TPM' 
CCLE_depMap_19Q1_TPM <- read.csv("~/R_projects/depmapr_csv_files/CCLE_depMap_19Q1_TPM.csv")
```
...
datasets. This dataset needs to be transposed to make to it conform to the 
orientation of the other datasets.

```{r, eval=FALSE, echo=T}
#load necessary libraries
library(dplyr, warn.conflicts = FALSE)

# We need to transpose our `D2_combined_genetic_dependency_scores` matrix but 
# we need to remove the first column `X` which is the list of genes because if
# that row is left in, it will make the remaining columns *factors* when the 
# dataframe is transposed. We will re-add the removed first column as the column
# names after the matrix is transposed. 

# the first 14 columns of `D2_combined_genetic_dependency_scores` are misnamed
# and need to be renamed without the `X` at the beginning
...
#transpose numeric columns of D2_combined_genetic_dependency_scores (rnaiNum)
tranD2 <- t(rnaiNum) 
...
names(metadata)[1] <-"depmapID"
names(metadata)[2] <- "cellLine"

# Rename first column of `mutationCalls` to "ID" and last column to "depmapID" 
names(mutationCalls)[1] <-"ID"
names(mutationCalls)[36] <-"depmapID"
```

While name improperly named columns have been renamed, not all depmap datasets
possess both "depmapID" or "cellLine". We will be performing *left_joins* and 
data transformations such that every dataset (except metadata) contains 
`depmapID` as the first column and `cellLine` as the second column.  
...
#View(depIDtoName)
#View(df1[c(1:10, 1:10)])
#View(df2[c(1:10, 1:10)])
#View(df3[c(1:10, 1:10)])
#View(df4[c(1:10, 1:10)])
```

We will ensure that there are no dataframes for which "depmapID" was added but 
may have a with NA value in that column, as to preserve the original size of 
those dataframes.
...
For transparency and reproducibility's sake, we will compare the attributes of 
the dataframes before and after cleaning. 
...
```{r}
# rnai data 
dim(tranD2) #tranD2 is the transpose of `D2_combined_genetic_dependency_scores`
# [1]   712 17309
dim(rnai)
# [1]   709 17311
...
devtools::use_data(crispr, overwrite = TRUE)
devtools::use_data(TPM, overwrite = TRUE)
devtools::use_data(RPPA, overwrite = TRUE)
devtools::use_data(metadata, overwrite = TRUE)
devtools::use_data(mutationCalls, overwrite = TRUE)
devtools::use_data(copyNumber, overwrite = TRUE)
```